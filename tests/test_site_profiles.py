...
class SiteProfileContractTestCase(unittest.TestCase):
    ...
    def test_gwdg_profile_keeps_rich_site_behaviour(self) -> None:
        ...
        self.assertIn("queue_short = 'scc-cpu'", text)
        self.assertIn("queue_standard = 'scc-cpu'", text)
        self.assertIn("queue_highmem = 'scc-cpu'", text)
    ...
    def test_viper_cpu_profile_enforces_site_and_login_node_contracts(self) -> None:
        ...
        self.assertIn("NF_HELPER_VIPER_COMPUTE_CONTROLLER", text)
        self.assertIn("== 'managed-slurm'", text)
        self.assertIn("viperLaunchHostShort.matches('vipc[0-9]+')", text)
        self.assertIn("(env('SLURM_JOB_ID') ?: '').matches('[0-9]+')", text)
    ...
    def test_raven_profile_combines_cpu_gpu_and_login_node_contracts(self) -> None:
        """Keep Raven CPU, GPU, internet, storage, and host policies explicit."""
        text = self.read_profile("raven")

        self.assertIn("profiles {", text)
        self.assertIn("raven {", text)
        self.assertIn("executor = 'slurm'", text)
        self.assertIn("clusterOptions = '--export=ALL'", text)
        self.assertIn("max_memory = 2048000.MB", text)
        self.assertIn("max_cpus = 72", text)
        self.assertIn("raven_max_cpus_ht = 144", text)
        self.assertIn("max_time = 24.h", text)
        self.assertIn("raven_slurm_queue_size = 250", text)
        self.assertIn("queueSize = params.raven_slurm_queue_size as int", text)
        self.assertIn("pollInterval = '30 sec'", text)
        self.assertIn("queueStatInterval = '30 sec'", text)
        self.assertIn("exitReadTimeout = '1 min'", text)
        self.assertIn("submitRateLimit = '5/1s'", text)
        ...
        self.assertIn("scratch = '$JOB_TMPDIR'", text)
        self.assertIn("withLabel: gpu", text)
        self.assertIn("cpus = { 18 * (params.gpus as int) }", text)
        self.assertIn("memory: 125000.MB * (params.gpus as int)", text)
        self.assertIn("cpus: 18 * (params.gpus as int)", text)
        self.assertIn("raven_gpu_constraint = 'gpu'", text)
        self.assertIn("gpu_cluster_options = null", text)
        self.assertIn("validGpuCounts = [1, 2, 4]", text)
        self.assertIn("validGpuConstraints = ['gpu', 'gpu-bw', 'no-gpu-bw']", text)
        self.assertIn('"--constraint=\\"${gpuConstraint}\\" --gres=gpu:a100:${gpuCount}"', text)
        self.assertIn("params.gpu_container_options ?: '--nv'", text)
        self.assertIn("apptainer_run_options = '--cleanenv'", text)
        self.assertIn("validRavenHosts.contains(ravenLaunchHostShort)", text)
        for host in ("raven01", "raven01i", "raven02", "raven02i", "raven03", "raven03i", "raven04", "raven04i"):
            self.assertIn(f"'{host}'", text)
        self.assertIn("NXF_APPTAINER_CACHEDIR", text)
        self.assertIn("ravenCacheDir.toString().startsWith('/ptmp/')", text)
        self.assertIn("raven_apptainer_module = 'apptainer/1.4.3'", text)
        self.assertIn("ociAutoPull = false", text)
        self.assertIn("runOptions = params.apptainer_run_options", text)
        self.assertIn("cleanup = false", text)
        self.assertNotIn("workDir =", text)
        self.assertNotIn("queue =", text)
